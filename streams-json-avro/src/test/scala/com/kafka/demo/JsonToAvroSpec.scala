...
final class JsonToAvroSpec extends AnyWordSpecLike with Matchers with EmbeddedKafkaStreams {
  ...
  private[this] implicit val embeddedKafkaConfig: EmbeddedKafkaConfig =
    EmbeddedKafkaConfig(kafkaPort = 7000, zooKeeperPort = 7001, schemaRegistryPort = schemaRegistryPort)
  ...
  private[this] implicit val keyAvroModelDeserializer: Deserializer[KeyAvroModel] =
    AvroCodec[KeyAvroModel].serde(schemaRegistryUrl).deserializer()

  private[this] implicit val valueAvroModelDeserializer: Deserializer[ValueAvroModel] =
    ...
    "verify topology" in {
      ...
      runStreams(Seq(inputTopic, outputTopic), topology) {
        ...
        val (resultKey, resultValue) = consumeFirstKeyedMessageFrom(outputTopic)(
          embeddedKafkaConfig,
          keyAvroModelDeserializer,
          valueAvroModelDeserializer
        )
