...
object JsonToAvroApp {
  ...
  protected[demo] def buildProperties(
  ...
  protected[demo] def buildTopology(
    schemaRegistryUrl: String,
    inputTopic: String,
    outputTopic: String
  ): Topology = {
    ...
    builder
      ...
      .map((keyString, jsonModel) =>
        (KeyAvroModel(keyString), ValueAvroModel(jsonModel.valueInt, jsonModel.valueString.toUpperCase))
      )
