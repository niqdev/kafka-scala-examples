...
final class KafkaSpec extends AnyWordSpecLike with Matchers with BeforeAndAfterAll {
  ...
  private[this] def produce(topic: String, records: Seq[(Option[String], String)]): Unit =
    kafkaServer.produce(
      topic,
  ...
  private[this] def consume(
    ...
    kafkaServer.consume(topic, size, timeoutMills, new StringDeserializer(), new StringDeserializer())

  private[this] def verifyConsumer(
    topic: String
